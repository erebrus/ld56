...
func _on_update(delta: float) -> void:
	...
	var squared_distance = new_position.distance_squared_to(head.tongue_position)
	var max_distance = _get_max_distance()
	if squared_distance > pow(max_distance, 2):
		target.global_position = head.tongue_position + head.tongue_position.direction_to(new_position) * max_distance
